...
class Bank(District):
	def __init__(self, rr, name, settings):
		...
		with self.nodes[addr] as n:
			...
			self.rr.AddBlock("B20",   self, n, addr, [(1, 2)])
			self.rr.AddBlock("B20.E", self, n, addr, [(1, 3)])
			self.rr.AddBlock("BOSWW", self, n, addr, [(1, 4)])
			self.rr.AddBlock("BOSWE", self, n, addr, [(1, 5)])
			self.rr.AddBlock("B11.W", self, n, addr, [(1, 6)])
			self.rr.AddBlock("B11",   self, n, addr, [(1, 7)])
			self.rr.AddBlock("B21.W", self, n, addr, [(2, 0)])
			self.rr.AddBlock("B21",   self, n, addr, [(2, 1)])
			self.rr.AddBlock("B21.E", self, n, addr, [(2, 2)])
			self.rr.AddBlock("BOSE",  self, n, addr, [(2, 3)])
