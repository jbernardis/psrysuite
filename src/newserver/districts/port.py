...
class Port(District):
	def __init__(self, rr, name, settings):
		...
		with self.nodes[PARSONS] as n:
			...
			sb = self.rr.AddBlock("P50.W",  self, n, addr, [(2, 3)])
			b = self.rr.AddBlock("P50",    self, n, addr, [(2, 4)])
			b.AddStoppingBlock(sb)
			sb = self.rr.AddBlock("P50.E",  self, n, addr, [(2, 5)])
			b.AddStoppingBlock(sb)
			
			sb = self.rr.AddBlock("P11.W",  self, n, addr, [(2, 6)])
			b = self.rr.AddBlock("P11",    self, n, addr, [(2, 7)])
			b.AddStoppingBlock(sb)
			sb = self.rr.AddBlock("P11.E",  self, n, addr, [(3, 0)])
			b.AddStoppingBlock(sb)
		...
		with self.nodes[PORTB] as n:
			...
			sb = self.rr.AddBlock("P32.W",  self, n, addr, [(2, 5)])	
			b = self.rr.AddBlock("P32",    self, n, addr, [(2, 6)])	
			b.AddStoppingBlock(sb)
			sb = self.rr.AddBlock("P32.E",  self, n, addr, [(2, 7)])	
			b.AddStoppingBlock(sb)
